...
// Итерация 41 — signed-upload для командной фотографии (шапка раздела «Команда»).
// Путь: covers/{team_id}/{uuid}.{ext} в bucket team-media. Кропа нет —
// изображение грузится как есть, обрезка по cover на UI.
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    const path = `covers/${org.team_id}/${crypto.randomUUID()}.${ext}`;
