...
### v0.8
- ✅ **Серия тренировок (итерация 72).** В форме тренировки блок «Повтор»: сколько тренировок создать (1–12), каждую неделю в этот же день и время, с превью дат. `POST /api/events` принимает `extra_starts_at`; даты считает клиент календарными днями, поэтому время не съезжает при переводе часов. События серии независимы — переносятся и отменяются по одному. Личное сообщение одно на серию, в группу анонсируется только ближайшая.
- ✅ **Анонс с экрана события (итерация 71).** Карточка «Анонс в группу» у организатора; после создания одиночного события открывается его экран. Подробности — [notifications.md](notifications.md).
- ✅ **Кнопка «Назад» (итерация 71).** Свой стек экранов сессии (`src/lib/nav-history.ts`) вместо ненадёжного `window.history.length`: при прямом входе на экран «Назад» ведёт на родительский экран.

### v0.5+
- Публичные события (`visibility = public`), лендинг события

## Не входит на PoC
- Настоящие recurring-правила (связанная серия с редактированием «всех следующих») — сейчас серия создаёт независимые события
- Календарный вид
