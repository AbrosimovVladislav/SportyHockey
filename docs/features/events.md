...
- ✅ **UX роли player (итерация 20).** Гейт по `useIsOrganizer(team_id)`. FAB на `/events` скрыт; `/events/new`, `/reschedule`, `/cancel` — редирект. На `/events/[id]`: большой `PlayerVoteBlock` под шапкой, блок «Состав и явка» скрыт, блок «Участники и взносы» → «Участники» без прогресс-бара взносов. На `/events/[id]/attendees`: title «Участники тренировки/игры», без дашборда оплат и без `ActionTile` «Был/Сдал»; `Я сдал` (payment-claim) для своей строки остаётся. Lineup, result, media — UI без изменений; API ослаблен до `requireUser + assertTeamMember`.
